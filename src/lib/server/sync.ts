...
export async function syncPuzzles(): Promise<{ fetched: number; added: number }> {
  ...
  const logId = db
    .prepare('INSERT INTO sync_log (sync_type, started_at) VALUES (?, ?)')
    .run('puzzles', Date.now()).lastInsertRowid;
...
export async function syncGames(): Promise<{ fetched: number; added: number }> {
  ...
  const logId = db
    .prepare('INSERT INTO sync_log (sync_type, started_at) VALUES (?, ?)')
    .run('games', Date.now()).lastInsertRowid;
...
function extractBlunders(game: RawGame, username: string): number {
  ...
  for (let i = 0; i < sanMoves.length && i < analysis.length; i++) {
    ...
    if (isUserMove && entry.judgment && entry.judgment.name !== 'Inaccuracy') {
      ...
      // Parse SAN to get the blunder move in UCI
